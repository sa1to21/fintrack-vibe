# Концепция: Долговые счета

## Основная идея
Реализовать долги и кредиты как подвид обычных счетов с флагом `isDebt`. Долговые счета отображаются в минус, не учитываются в общей статистике и могут погашаться через трансферы.
...
### 1. Создание долгового счета (ManageAccountsPage)
- Чекбокс/тумблер: **"Долговой счет"**
- При активации показываются доп. поля:
  - Кредитор (обязательное)
  - Начальная сумма долга (обязательное)

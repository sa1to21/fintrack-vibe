  ...
  module V1
    class TransfersController < BaseController
      ...
      def create
        ...
        # Проверка: счёт задолженности не может уйти в плюс
        if to_account.is_debt
          new_balance = to_account.balance + amount
          if new_balance > 0
            return render json: {
              error: 'Счёт задолженности не может иметь положительный баланс',
              details: ["\"#{to_account.name}\" — счёт задолженности. Максимальная сумма для пополнения: #{(-to_account.balance).round(2)} #{to_account.currency}"]
            }, status: :unprocessable_entity
          end
        end
      ...
      def get_debt_repayment_category
        # Находим или создаем категорию "Погашение задолженности"
        current_user.categories.find_or_create_by!(

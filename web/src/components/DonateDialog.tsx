...
export function DonateDialog({ open, onOpenChange }: DonateDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[90vw] max-w-sm border-pink-200 bg-gradient-to-br from-white to-pink-50/30">
        <DialogHeader>
              ...
              <CardContent className="p-3">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex-1 min-w-0 overflow-hidden">
                    <h3 className="font-medium text-slate-800 text-sm mb-0.5 truncate">
                      {method.name}
                    </h3>
                    <p className="text-xs text-slate-500 mb-0.5 truncate">{method.label}</p>
                  ...
                  <Button
                    ...
                    className={`flex-shrink-0 min-w-[36px] h-9 ${
                      copiedIndex === index
